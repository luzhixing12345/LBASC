...
class ScopedSymbolTable(object):
    ...
    def lookup(self, name, current_scope_only=False):
        self.log(f'Lookup: {name}. (Scope name: {self.scope_name})')        # 'symbol' is either an instance of the Symbol class or None
...
class SemanticAnalyzer(NodeVisitor):
    ...
    def visit_ProcedureCall(self, node):
        ...
        proc_name = node.proc_name
        procedureCall = self.current_scope.lookup(proc_name)
        if len(procedureCall.params) != len(node.actual_params):
